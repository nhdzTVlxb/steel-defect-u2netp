...
class self_net(nn.Module):
    ...
    #         # 手动初始化不匹配的层（seg_heads.*.conv_out）
    #         for i in range(3):
    #             nn.init.xavier_uniform_(net.seg_heads[i].conv_out.weight)
    #             if net.seg_heads[i].conv_out.bias is not None:
    #                 nn.init.zeros_(net.seg_heads[i].conv_out.bias)
