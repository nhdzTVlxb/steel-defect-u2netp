...
import random
import torch
MODEL_NAME = 'segnext_AUG_b_l_D_b' 
...
model= self_net().cuda()
# net.load_state_dict(torch.load(r'saved_models\unet\unet_0.7546812412982268_39.pth'))  # 加载权重
# ------- 4. define optimizer --------
...
for epoch in range(NUM_EPOCHS):
    ...
    for data in tqdm(salobj_dataloader, total=len(salobj_dataloader), desc=f"Epoch {epoch+1}/{NUM_EPOCHS} - Training"):
        ...
        # Compute loss
        loss = 0.4*bce_loss(outputs, labels.squeeze()) + 0.3*iouloss(outputs, labels.squeeze())+0.1*diceloss(outputs, labels.squeeze())+0.2*Boundary_loss(outputs, labels.squeeze())
    ...
    with torch.no_grad():
        for data in tqdm(salobj_valdataloader, total=len(salobj_valdataloader), desc=f"Epoch {epoch+1}/{NUM_EPOCHS} - Validation"):
            ...
            # Compute loss
            loss = bce_loss(outputs, seg_masks.squeeze())
            val_loss += loss.item()
