...
class SegmentationMetric(object):
    ...
    def classPixelAccuracy(self):
        ...
        return classAcc#np.nanmean(classAcc)  # 返回的是一个列表值，如：[0.90, 0.80, 0.96]，表示类别1 2 3各类别的预测准确率
...
if __name__ == '__main__':
    imgPredict = torch.tensor([0, 0, 1, 1, 2, 2]).cuda() # 可直接换成预测图片
    imgLabel = torch.tensor([0, 0, 1, 1, 2, 2]).cuda() # 可直接换成标注图片
    ...
    # a=compute_metrics2(imgPredict, imgLabel, num_classes=3)
    # print(a)
